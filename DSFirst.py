...
""" 
    embedding使用sentence_transformers把文字变成384维向量
...
class VectorMemory:
    def __init__(self,save_path:str="memory"):
        ...
        self.encoder=SentenceTransformer('all-MiniLM-L6-v2')  #选择模型all-MiniLM-L6-v2，离线，80mb
...
# 创建ManageMeomory类实例manager
manager=ManageMemory("memory")
# 每次检索3条相关向量，Top_k=3
Top_k=3
print("可用命令：/list,/creat 项目名,/use 项目名,/delete 项目名")
print()
...
while True:
    ...
    if user_input.startswith("/"):
        ...
        continue  # ★ 处理完命令就跳过下面的 AI 对话

    if manager.current is None:
        print("当前未选择项目，请先选择项目，可使用命令：/help 查看帮助")
        continue

    # 找到相关向量，但是存在一个问题，找到的向量不一定是相关的
    history=manager.search(user_input,Top_k)
    ...
    #把本轮对话的uesr_input和AI_reply加入到memory中
